(function() {
  ...
  function ellipticFi(φ, ψ, m) {
    ...
    if (r) {
      ...
      return [ ellipticF(Math.atan(1 / Math.sqrt(cotλ2)), m) * sgn(φ), ellipticF(Math.atan(Math.sqrt(cotλ2 / cotφ2 - 1) / m), 1 - m) * sgn(ψ) ];
    }
  ...
  function armadillo(φ0) {
    var sinφ0 = Math.sin(φ0), cosφ0 = Math.cos(φ0), tanφ0 = Math.tan(φ0), k = (1 + sinφ0 - cosφ0) / 2;
    function forward(λ, φ) {
      var cosφ = Math.cos(φ), cosλ = Math.cos(λ /= 2), φ1 = -Math.atan2(cosλ, tanφ0);
      return [ (1 + cosφ) * Math.sin(λ), (φ > φ1 - .001 ? 0 : -1) + k + Math.sin(φ) * cosφ0 - (1 + cosφ) * sinφ0 * cosλ ];
    }
    return forward;
  }
  function armadilloProjection() {
    var φ0 = π / 9, tanφ0 = Math.tan(φ0), m = projectionMutator(armadillo), p = m(φ0), stream_ = p.stream;
    p.parallel = function(_) {
      if (!arguments.length) return φ0 / π * 180;
      return m(φ0 = _ * π / 180);
    };
  ...
  function bonne(φ0) {
    ...
    function forward(λ, φ) {
      var ρ = cotφ0 + φ0 - φ, E = λ * Math.cos(φ) / ρ;
  ...
  function conicConformal(φ0, φ1) {
    ...
    }, n = Math.log(cosφ0 / Math.cos(φ1)) / Math.log(t(φ1) / t(φ0)), F = cosφ0 * Math.pow(t(φ0), n) / n;
    function forward(λ, φ) {
  ...
  function conicEquidistant(φ0, φ1) {
    var cosφ0 = Math.cos(φ0), n = (cosφ0 - Math.cos(φ1)) / (φ1 - φ0), G = cosφ0 / n + φ0;
    function forward(λ, φ) {
      var ρ = G - φ;
